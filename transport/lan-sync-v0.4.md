...
6. the steady-state storage-command hot path over the long-lived WebRTC control channel.
...
Nearbytes LAN Sync v0.4 MUST use:
...
8. direct low-latency delivery over a long-lived peer channel during healthy steady-state connectivity.
...
1. `pv`: LAN protocol version string, currently `0.4`
2. `peer`: peer identity string
3. `alpn`: transport profile token, currently `nearbytes-lan/0.4`
...
The transport profile token for v0.4 is:
...
1. one reliable long-lived control data channel for hello, signaling completion, storage commands, cursor sync, and recovery hints;
2. reliable multiplexed request-response carriage for event and block fetches;
3. concurrent object fetches without blocking unrelated control traffic.
...
## 9. Steady-State Storage Commands

Healthy peers SHOULD use direct storage commands over the long-lived control channel as the normal low-latency path.

Rules:

1. a newly observed event SHOULD trigger an immediate event storage command;
2. a newly observed block SHOULD trigger an immediate block storage command;
3. the receiver decides whether it wants the announced object;
4. raw bytes are fetched only when the receiver is missing the object;
5. this hot path is defined in `transport/storage-commands-v0.1.md`.

## 10. Receiver-Driven Transfer Rule
...
2. a sender MAY advertise observations, heads, hints, inventories, or storage commands;
3. advertising an observation, inventory entry, or storage command is the protocol-level equivalent of asking the receiver whether it wants object `X`;
...
4. in healthy steady state, exchange direct storage commands for new observations;
5. if commands are missed or state is uncertain, pull unseen typed observations;
6. determine which events and blocks are actually wanted locally;
7. request only the missing events and blocks;
8. validate and store them;
9. run inventory anti-entropy as recovery if cursors are missing or stale.
...
6. storage-command loss or duplication MUST be repaired through observation-log replay and anti-entropy.

## 13. Current Replacement Rule
...
3. WebRTC plus DNS-SD is the normative v0.4 profile for Nearbytes LAN sync;
