...
6. **CRDT-trivial.** Two replicas observing the same set of events in the same total order MUST produce the same materialized state. In v0.4 the total order is defined externally (timestamp → log position → primary path → event hash) and is the same one used by the timeline. v0.5 changes the ordering layer to use semantic `blockRefs`; the materializer rules remain the same unless the v0.5 spec says otherwise.
...
7. **v0.5 causal mode.** `application/file-events-v0.5.md` uses the existing cleartext `blockRefs` field, not a new typed envelope field, to carry the FILES observed-log-head dependency. v0.5 total order is a deterministic topological sort of that application-defined event parent graph, with timestamps and event hashes used only to choose among currently ready events. The materializer and §4 conflict rules are written to be independent of how the total order is computed, so adopting causal order is primarily a sort-key change. See `application/blockrefs-v0.1.md` for the generic `blockRefs` discipline and `engineering/hash-evolution-v1.md` for major-version migration policy.
