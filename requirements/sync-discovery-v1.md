...
| DISC-24 | LAN discovery MUST NOT open sync to advertisers whose `prof` is not in `config.friends`, MUST ignore announcements whose `prof` matches **any** served local profile (loopback), and MUST initiate the TCP dial as the served local profile that the advertised `prof` is **dialed from**, i.e. the implementation's outbound socket targets the advertiser's announced `syncPort` while the local handshake signs with the **active** served profile (the served profile chosen by the user via `profile use`). |
