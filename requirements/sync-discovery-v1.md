...
| DISC-27 | **Node identity is anchored in `dataDir`, not in the OS process.** Implementations MUST treat `peerId` as a per-`dataDir` stable random identifier (created on first start, persisted under the storage root) so that two processes sharing the same on-disk log evaluate to the **same** `peerId` and are filtered as loopback by DISC-24, never as siblings. Implementations MUST additionally take an exclusive advisory file lock on `dataDir` at start and release it at stop; a second `start()` against a locked `dataDir` MUST fail loudly with a message naming the existing holder. Purely in-memory deployments (no `dataDir`) MAY use a per-process random `peerId`. |
