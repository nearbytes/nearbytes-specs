...
- **SYNC-19–SYNC-23** — *Persistent fetch cursors and inventory repair.* Receivers persist per-remote profile@instance cursors into the remote reception stream, and filesystem sync engines repair `sync/reception.jsonl` from all stored blocks/events on startup so fresh devices can bootstrap without manual reindexing.
- **SYNC-60–SYNC-62** — *New-machine recovery.* Full historical bootstrap is normative: a fresh `dataDir` converges from a reachable complete friend/sibling via inventory-repaired reception pagination plus ordinary `have`/`want`.
