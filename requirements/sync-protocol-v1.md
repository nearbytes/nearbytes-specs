...
| Rule | Requirement |
|------|-------------|
| SYNC-30 | Block bytes MUST be sent either as a continuous raw stream after stream-begin (see SYNC-33) or as binary block-chunk frames that carry raw byte slices plus hash/offset/total metadata. Base64, base64url, JSON-in-JSON, or any other **content encoding of block bytes is PROHIBITED**. |
...
| SYNC-33 | In raw-stream mode, after `want` for a block, the sender MUST emit one **stream-begin** frame (hash + total length) then **`total` contiguous raw bytes** on the duplex — no per-chunk length prefixes, no splitting the file into framed segments, and no interleaved control/event frames until those `total` bytes are complete. In framed-chunk mode, each block-chunk frame MUST carry the block hash, byte offset, total length, and raw chunk bytes; chunks for the same hash MUST be sent in ascending offset order, and chunks for distinct hashes MAY be interleaved. |
| SYNC-34 | Receivers MUST read each block directly into its destination buffer or per-block disk sink (one allocation/sink per block). Senders MAY split raw streams across multiple transport writes to respect transport atomic-write limits; framed-chunk senders MUST NOT change the per-frame hash/offset/total semantics. |
| SYNC-35 | Pump writes MAY use `subarray` slices over the source `ArrayBuffer`; each transport write MUST stay below the active transport's maximum atomic write size. Framed-chunk mode SHOULD be used on transports where a raw block stream would prevent safe multiplexing or exceed atomic-write limits. Avoid base64/JSON/text encodings of block ciphertext (SYNC-30). |
