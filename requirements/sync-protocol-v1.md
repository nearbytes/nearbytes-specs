...
| SYNC-21c | On each new friend-session attach, the node MUST proactively send at least one `have` page covering the tail of its own reception journal (reference: last 256 `seq` entries) in addition to mutual `delta`/`subscribe`, so a peer that comes online later still receives writes that landed while it was offline. |
