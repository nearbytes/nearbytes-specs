...
| Rule | Requirement |
|------|-------------|
| SYNC-60 | **New-machine recovery.** A fresh `dataDir` MUST be able to converge to the union of objects reachable from online friends without manual reindex steps. Today, cursor pagination over a locally empty reception journal can miss objects that exist on disk at a peer but are not re-advertised; this is under active redesign. SYNC-16–SYNC-17 mitigate **partial** catch-up when a child event arrived before its parent, but do not replace full historical bootstrap. |
