...
| Rule | Requirement |
|------|-------------|
| SYNC-01 | Each association MUST serialize all outbound writes on one duplex. Control/event frames MUST NOT be written concurrently with each other or with a block stream. |
...
| SYNC-33 | After `want` for a block, the sender MUST emit one **stream-begin** frame (hash + total length) then **`total` contiguous raw bytes** on the duplex — no per-chunk length prefixes, no splitting the file into framed segments, and no interleaved control/event frames until those `total` bytes are complete. |
| SYNC-34 | Receivers MUST read the byte stream directly into the destination buffer (one allocation per block). Senders MAY split the raw byte stream across multiple transport writes to respect transport atomic-write limits, but MUST NOT add per-chunk sync framing or change the stream-begin `total`. |
| SYNC-35 | Pump writes MAY use `subarray` slices over the source `ArrayBuffer`; each transport write MUST stay below the active transport's maximum atomic write size. Avoid base64/JSON/text encodings of block ciphertext (SYNC-30). |
...
## 8. Association health and lifecycle

Quiet associations that are waiting for unsolicited push `have` hints (SYNC-10c) MUST remain open indefinitely. Implementations MUST nevertheless detect **hung** associations — transports that appear live but make no protocol progress — and **rotate** quiescent associations after a bounded age so the per-triple session slot (SYNC-06) can be reclaimed for a fresh handshake. These mechanisms MUST NOT use wall-clock timers to drive anti-entropy repair (SYNC-17 remains timer-free on the wire).

| Rule | Requirement |
|------|-------------|
| SYNC-63 | Every TCP socket backing a sync association MUST enable kernel TCP keepalive so half-open legs are torn down without requiring application traffic. Reference: `setKeepAlive(true)` with an initial delay of 60 s on Node.js. This probes transport liveness during push-wait idle and MUST NOT be confused with an application idle timeout. |
| SYNC-64 | Implementations MUST arm **in-flight stall budgets** only while work is outstanding on an association: (a) after emitting `want` for a block hash, until `block-stream-begin` or claim release; (b) during a partial inbound block stream, until the stream completes or is discarded; (c) during an outbound block pump for that association; (d) during a local resume-walk page (`delta` sent, matching resume `have` not yet processed). When a budget expires, the implementation MUST close the association and emit `peer-stalled` (OBS-64). Reference budgets: 120 s for (a)(b)(d); 300 s for (c) on large blocks. |
| SYNC-65 | Implementations MAY close a **quiescent** association after a maximum session age when no in-flight work remains (no outstanding block `want` claims, no partial block stream, no resume page in flight, no outbound pump on that association). Reference: 600 s for short-lived engines; long-lived daemons MAY use longer budgets. Closure MUST emit `peer-stalled` with reason `session-rotation` before `peer-disconnected`. Persistent fetch cursors (SYNC-19) make reconnect catch-up efficient. |
| SYNC-66 | Closing an association for stall or rotation MUST release per-hash `want` reservations held by that session (SYNC-14) so a later association can re-`want`. |
| SYNC-67 | LAN mDNS outbound discovery MUST allow re-dial to the same `(remote profile, remote instance, host, port)` after an association to that endpoint closes, so rotation and stall recovery can establish a fresh TCP leg without requiring a process restart. |
